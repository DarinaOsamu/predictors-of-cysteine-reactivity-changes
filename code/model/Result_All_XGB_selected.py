...
def main():
    ...
    '''
    #获取反应性改变的数据
    Changed_validation=dataset.loc[dataset['type']!='Unchanged']
    #查看样本数和特征数
    #print(Changed_validation.iloc[0:3,:])
    print(Changed_validation.shape)
    print('-' * 30)

    Changed_types=Changed_validation['type']
    Changed_validation=Changed_validation.iloc[:,3:]
    #print(Changed_validation.iloc[0:3, :])

    #划分X，转换y的标签值
    X2_validation=Changed_validation
    ...
    #Result_all.to_excel(excel_writer=r'../../result/Result_all.xlsx', index=False)
    ##############################
