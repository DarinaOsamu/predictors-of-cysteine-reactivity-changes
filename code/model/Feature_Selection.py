...
def main():
    ...
    #不进行SMOTE处理
    X1_ST, y1_ST=X1_train_test, y1_train_test
    #读取保存的标化器
    ...
    #pickle.dump(optimal_enet1, open('../../result/feature_selector/EN_baseline_PosNeg.pkl','wb'))

    #读取得到的最优弹性网络
    optimal_enet1=pickle.load(open('../../result/feature_selector/EN_baseline_PosNeg.pkl','rb'))
    ...
    # 保存选择器
    #pickle.dump(selector1, open('../../result/feature_selector/EN_Selector_PosNeg.pkl','wb'))
    ...
    X2_ST, y2_ST = Sampler.fit_resample(X2_train_test, y2_train_test)
    print('综合采样后，训练集 ST_y2中的分类情况：{}'.format(pd.value_counts(y2_ST)))
    
    #读取保存的标化器
    scaler2=pickle.load(open('../../result/standard_scaler/standard_SMOTE_IncreDecre.pkl','rb'))
    SS_X2_ST=scaler2.fit_transform(X2_ST)
    SS_X2_ST=pd.DataFrame(SS_X2_ST)
    
    #训练弹性网络
    #optimal_enet2=ElasticNet(SS_X2_ST,y2_ST)
    #pickle.dump(optimal_enet2, open('../../result/feature_selector/EN_SMOTE_IncreDecre.pkl','wb'))

    #读取得到的最优弹性网络
    optimal_enet2=pickle.load(open('../../result/feature_selector/EN_SMOTE_IncreDecre.pkl','rb'))
    coef_2 = optimal_enet2.coef_
    # Plot coef 
    plt.plot(coef_2)
    plt.show()
    ...
    #生成选择器
    selector2=Feature_Selector(optimal_enet2,SS_X2_ST,y2_ST)
    #pickle.dump(selector2, open('../../result/feature_selector/EN_Selector_IncreDecre.pkl', 'wb'))
    
    #读取得到的选择器
    selector2=pickle.load(open('../../result/feature_selector/EN_Selector_IncreDecre.pkl','rb'))
    print(selector2.get_support())
    
    X2_embedded = selector2.transform(SS_X2_ST)
    print(SS_X2_ST.shape,X2_embedded.shape)
    '''
    '''
