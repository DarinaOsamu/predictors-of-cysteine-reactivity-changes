...
def Assessment(clf,X,y,predicted):
    ...
    # 绘制混淆矩阵
    metrics.ConfusionMatrixDisplay.from_estimator(clf, X, y, cmap='Reds')
    plt.show()
...
def main():
    ...
    # 读取保存的分类器，预测独立验证集的y
    clf1 = pickle.load(open('../../result/classifier/LogisticRegression_selected_PosNeg.pkl', 'rb'))
    ...
    #读取保存的分类器，预测独立验证集的y
    clf2=pickle.load(open('../../result/classifier/LogisticRegression_selected_IncreDecre.pkl','rb'))
