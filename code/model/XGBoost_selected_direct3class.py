...
def main():
    ...
    # SMOTETomek 综合抽样
    print('不经过任何采样处理的原始 y2中的分类情况：{}'.format(pd.value_counts(y1_train_test)))
    ...
    # 保存标化器
    #pickle.dump(scaler1, open('../../result/standard_scaler/standard_SMOTE_direct3class.pkl','wb'))
    ...
    # 绘制学习曲线
    plot_learning_curve(optimal_XGB1,"XGB-3class", SS_X1_train_test,  y1_train_test)

    # 保存最优模型
    pickle.dump(optimal_XGB1, open('../../result/classifier/XGBoost_selected_direct3class.pkl','wb'))
