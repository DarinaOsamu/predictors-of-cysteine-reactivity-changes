...
def XGB_classifier(X_train, y_train):
    ...
    ##############################
    # 调用分类器
    clf = XGBClassifier(random_state=0, n_estimators=500,learning_rate=0.3)
    ...
    #clf.set_params(n_estimators=82)
...
def main():
    ...
    # 保存最优模型
    pickle.dump(optimal_XGB1, open('../../result/classifier/XGBoost_selected_PosNeg.pkl','wb'))
    '''
    ##############################
    ...
    # 保存最优模型
    pickle.dump(optimal_XGB2, open('../../result/classifier/XGBoost_baseline_selected_IncreDecre.pkl','wb'))
